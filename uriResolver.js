...
class BuiltInResolvers {
   ...
   */
  static fileSystemResolver(basePath = './events') {
    const fs = require('fs').promises;
    const path = require('path');

    return async (parsed) => {
      const filePath = path.join(basePath, parsed.date, parsed.source, `${parsed.slug}.json`);
      
      try {
        const data = await fs.readFile(filePath, 'utf8');
        return JSON.parse(data);
      } catch (error) {
        throw new Error(`Event not found: ${filePath}`);
      }
    };
  }
